    ...
    <b-tabs class="has-text-full-03" style="height: 100%" :value="firstAppName">
      <b-tab-item v-for="(service, key) in configData.services" :key="key" :label="key" :value="key" @click="current_service = key">
        <ValidationObserver :ref="key + 'valida'">
          <b-field grouped>
            <ValidationProvider class="is-flex-grow-1 mr-3" v-slot="{ errors, valid }" name="Image0" rules="required">
              <b-field :label="$t('Docker Image') + ' *'" :message="$t(errors)" :type="{ 'is-danger': errors[0], 'is-success': valid }" class="mb-3">
                <b-input :key="service.image" :readonly="state == 'update' || serviceStableVersion !== ''" :value="getFirstField(service.image)" :placeholder="$t('e.g.,hello-world:latest')" @input="(V) => changeIcon(V)" @blur="
										(E) => {
            ...
            <ValidationProvider v-slot="{ errors, valid }" name="Image1" rules="required">
              <b-field :label="$t('Tag')" :message="$t(errors) || '123'" :type="{ 'is-danger': errors[0], 'is-success': valid }"><b-dropdown aria-role="menu" trap-focus>
                  <template #trigger>
                    <b-input icon-pack="casa" icon-right="down-outline" class="is-flex-grow-1" :value="getLateField(service.image)" @input="
												(V) => {
                  ...
                  <b-dropdown-item key="stable" v-show="serviceStableVersion !== '' && firstAppName === key" @click="
									() => {
          ...
          <ValidationProvider v-slot="{ errors, valid }" name="composeAppName" rules="required">
            <b-field :label="$t('App Name') + ' *'" :message="$t(errors)" :type="{ 'is-danger': errors[0], 'is-success': valid }">
              <b-input :placeholder="$t('e.g.,Your App Name')" :value="ice_i18n(configData['x-casaos'].title)" @blur="(E) => (configData['x-casaos'].title.custom = E.target._value)"></b-input>
            </b-field>
          </ValidationProvider>
          ...
          <b-field v-if="key === firstAppName" :label="$t('Icon URL')">
            <p class="control">
              <span class="button is-static container-icon">
                <b-image :key="appIcon" :src="appIcon" :src-fallback="require('@/assets/img/app-icons/default.svg')" class="is-32x32" ratio="1by1"></b-image>
              </span>
            </p>
          ...
          <ports v-if="showPorts(service)" v-model="service.ports" :ports_in_use="ports_in_use" :showHostPost="showHostPort(service)"></ports>

          <volumes-input-group v-model="service.volumes" :label="$t('Volumes')" :message="$t('No volumes now, click “+” to add one.')" type="volume">
          </volumes-input-group>
          <env-input-group v-model="service.environment" :label="$t('Environment Variables')" :message="$t('No environment variables now, click “+” to add one.')">
          </env-input-group>
          <input-group :devices="service.devices" :label="$t('Devices')" :message="$t('No devices now, click “+” to add one.')" type="device">
          </input-group>
          <commands-input v-model="service.command" :label="$t('Container Command')" :message="$t('No commands now, click “+” to add one.')">
          </commands-input>
          ...
          <b-field :label="$t('Memory Limit')" class="mb-5">
            <vue-slider :max="totalMemory" :min="memory_min" class="mx-2" :marks="true" :data="markData" :value="service.deploy.resources.limits.memory | duplexDisplay" @change="(v) => (service.deploy.resources.limits.memory = v)"></vue-slider>
          </b-field>
...
import Ports from "@/shared/forms/Ports.vue";
import EnvInputGroup from "@/shared/forms/EnvInputGroup.vue";
import CommandsInput from "@/shared/forms/CommandsInput.vue";
import InputGroup from "@/shared/forms/InputGroup.vue";
import VolumesInputGroup from "@/shared/forms/VolumesInputGroup.vue";
...
::v-deep .vue-slider-mark-step {
	...
	height: 100%;
	border-radius: 50%;
	-webkit-box-shadow: 0 0 0 2px var(--color-border-strong);
