...
// @Summary full SMART info for a drive (uncached)
// @Produce  application/json
// @Accept application/json
// @Tags disk
// @Security ApiKeyAuth
// @Success 200 {string} string "ok"
// @Router /disks/smart [get]
...
func PostDiskSmartTest(c *gin.Context) {
	...
	if err := service.MyService.Disk().SmartTest(path, testType); err != nil {
...
// @Summary get a drive's configured standby/spindown timer, in minutes
// @Produce  application/json
// @Accept application/json
// @Tags disk
// @Security ApiKeyAuth
// @Success 200 {string} string "ok"
// @Router /disks/standby [get]
func GetDiskStandby(c *gin.Context) {
	path := c.Query("path")
	if len(path) == 0 {
		c.JSON(common_err.CLIENT_ERROR, model.Result{Success: common_err.INVALID_PARAMS, Message: common_err.GetMsg(common_err.INVALID_PARAMS)})
		return
	...
	minutes := service.MyService.Disk().GetStandby(path)
	c.JSON(common_err.SUCCESS, model.Result{Success: common_err.SUCCESS, Message: common_err.GetMsg(common_err.SUCCESS), Data: map[string]int{"minutes": minutes}})
}

// @Summary set a drive's standby/spindown timer, in minutes (0 disables it)
// @Produce  application/json
// @Accept application/json
// @Tags disk
// @Security ApiKeyAuth
// @Success 200 {string} string "ok"
// @Router /disks/standby [put]
type standbyRequest struct {
	Path    string `json:"path" form:"path"`
	Minutes int    `json:"minutes" form:"minutes"`
}

func PutDiskStandby(c *gin.Context) {
