...
func InitV1Router() http.Handler {
	...
	}))
	{
		...
		{
			v1ContainerGroup.GET("/all", v1.GetAllContainersWithUpdates)
			v1ContainerGroup.GET("/usage", v1.AppUsageList)
			v1ContainerGroup.GET("/:id", v1.ContainerUpdateInfo)
			v1ContainerGroup.GET("/:id/compose", v1.ToComposeYAML)
			v1ContainerGroup.GET("/:id/logs", v1.ContainerLog)
			v1ContainerGroup.GET("/networks", v1.GetDockerNetworks)
			...
			v1ContainerGroup.PUT("/auto-update/config", v1.SetAutoUpdateConfig)
			v1ContainerGroup.POST("/check-all-updates", v1.CheckAllContainersUpdate)

			v1ContainerGroup.PUT("/:id", v1.UpdateSetting)
			v1ContainerGroup.PUT("/:id/state", v1.ChangAppState)
			v1ContainerGroup.DELETE("/:id", v1.UninstallApp)
		}
	}
